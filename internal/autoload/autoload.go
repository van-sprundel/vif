...
func generateIncludePaths(paths []string) string {
	...
	b.WriteString("<?php\n\n")
	b.WriteString("// include_paths.php @generated by Composer\n\n")
...
func generateReal(hash string, hasFiles bool, hasIncludePaths bool, prepend bool) string {
	...
	if hasIncludePaths {
		...
		s = strings.Replace(s, "<INCLUDE_PATHS>", "", 1)
	}
	if hasFiles {
	...
	if prepend {
		s = strings.ReplaceAll(s, "<PREPEND>", "true")
	} else {
		s = strings.ReplaceAll(s, "<PREPEND>", "false")
	}
	s = strings.ReplaceAll(s, "<HASH>", hash)
	return s
}
